...
**Chat header badge** (`components/aula/TopicBadge.tsx`, consumed from `app/aula/AulaClient.tsx`): `AulaClient` already resolves `topicData` client-side via `getTopicByKey(topic)`. Add a parallel client-side lookup, `getNpcForTopic(topic)` from the newly-shared `lib/npcs.ts` — purely derived from the topic key already known client-side, no new fetch. When it resolves to an NPC (implying this session's methodology is roleplay on an NPC-mapped topic), pass `"${npc.emoji} Com ${npc.name}"` as the `topic` prop to `TopicBadge` instead of the topic's own label. This is a display-only derivation; it does not need to know the session's actual `npc_key`, since the mapping is static and 1:1 per topic.
...
- `TopicBadge`/`AulaClient` tests (modified): case confirming the badge shows the NPC's name+emoji when the resolved topic has a matching NPC.
